...
export default function ArtistTracker() {
  ...
  const renderIcon3D = (featureId: string) => {
    ...
    const containerClasses = "relative w-full h-full bg-zinc-800/50 backdrop-blur-xl rounded-2xl border border-zinc-700/50 shadow-2xl flex items-center justify-center group-hover:border-[#FFD60A]/40 transition-all duration-300";
      ...
      case "wallet":
        return (
          <div className={baseClasses} style={{ perspective: "200px", transformStyle: "preserve-3d" }}>
            {/* Glow */}
            <div className={`${glowClasses} bg-[#FFD60A]/20`} style={{ transform: "translateZ(-20px)" }} />
            ...
            <div className={containerClasses} style={{ transform: "translateZ(0px)" }}>
              ...
              <div className="relative" style={{ transform: "translateZ(15px)" }}>
                {/* Wallet base */}
                <div className="w-8 h-6 bg-zinc-700 border border-zinc-600/50 shadow-lg transform -rotate-12" 
                     style={{ transformStyle: "preserve-3d" }}>
                  {/* Wallet depth */}
                  <div className="absolute inset-0 bg-zinc-800 transform translate-x-1 translate-y-1" 
                       style={{ transform: "translateZ(-2px)" }} />
                  {/* Cards inside */}
                  <div className="absolute top-1 left-1 w-6 h-1 bg-[#FFD60A]" />
                  <div className="absolute top-2.5 left-1 w-5 h-1 bg-[#FFD60A]/60" />
                </div>
              </div>
            </div>
          </div>
        ...
        return (
          <div className={baseClasses} style={{ perspective: "200px", transformStyle: "preserve-3d" }}>
            ...
            <div className={containerClasses} style={{ transform: "translateZ(0px)" }}>
              ...
              <div className="relative" style={{ transform: "translateZ(15px)" }}>
                {/* Phone body */}
                <div className="w-5 h-8 bg-zinc-700 border border-zinc-600 shadow-lg" 
                     style={{ transformStyle: "preserve-3d" }}>
                  {/* Screen */}
                  <div className="absolute inset-1 bg-zinc-800">
                    {/* NFC waves */}
                    <div className="absolute -right-3 top-1/2 -translate-y-1/2">
                      <div className="w-4 h-4 border-2 border-[#FFD60A] animate-ping opacity-60" />
                      <div className="absolute inset-0 w-3 h-3 border-2 border-[#FFD60A] animate-ping opacity-40" style={{ animationDelay: '0.5s' }} />
                    </div>
                  </div>
                </div>
              </div>
        ...
        return (
          <div className={baseClasses} style={{ perspective: "200px", transformStyle: "preserve-3d" }}>
            ...
            <div className={containerClasses} style={{ transform: "translateZ(0px)" }}>
              ...
              <div className="relative" style={{ transform: "translateZ(15px)" }}>
                {/* QR Code base */}
                <div className="w-7 h-7 bg-white p-1 shadow-lg border border-zinc-400" 
                     style={{ transformStyle: "preserve-3d" }}>
                  {/* QR Pattern */}
                  <div className="w-full h-full bg-black relative overflow-hidden">
                    ...
                    <div className="absolute top-1 left-1 w-4 h-1 bg-white" />
                    <div className="absolute top-2 left-0 w-1 h-3 bg-white" />
                    <div className="absolute top-2 right-1 w-2 h-1 bg-white" />
                    <div className="absolute bottom-1 left-2 w-3 h-1 bg-white" />
                    
                    {/* Scan line effect */}
                    <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#FFD60A]/40 to-transparent h-full w-full animate-pulse" />
                  </div>
                  
                  {/* Scan corners */}
                  <div className="absolute -top-1 -left-1 w-2 h-2 border-l-2 border-t-2 border-[#FFD60A]" />
                  <div className="absolute -top-1 -right-1 w-2 h-2 border-r-2 border-t-2 border-[#FFD60A]" />
                  <div className="absolute -bottom-1 -left-1 w-2 h-2 border-l-2 border-b-2 border-[#FFD60A]" />
                  <div className="absolute -bottom-1 -right-1 w-2 h-2 border-r-2 border-b-2 border-[#FFD60A]" />
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="bg-black rounded-[2.5rem] overflow-hidden border border-gray-700/50">
                  ...
                  <div className="px-3 sm:px-4 pb-3 sm:pb-4 space-y-3 sm:space-y-4">
                    ...
                    <div className="relative space-y-2 sm:space-y-3" style={{ transformStyle: "preserve-3d" }}>
                      ...
                      <div 
                        className="absolute top-8 left-2 right-2 bg-gradient-to-r from-blue-600 to-cyan-500 p-4 shadow-lg transform rotate-1 z-20 opacity-80 transition-transform duration-300"
                        style={{ 
                          transformStyle: "preserve-3d",
                      ...
                      <div 
                        className="absolute top-12 left-4 right-4 bg-gradient-to-r from-green-600 to-teal-500 p-4 shadow-lg transform -rotate-1 z-10 opacity-60 transition-transform duration-300"
                        style={{ 
                          transformStyle: "preserve-3d",
